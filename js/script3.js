...
function endLine(e) {
    ...
    // Solo permitir unir imágenes si son iguales (según el alt de las imágenes) y no están usadas
    ...
    if (startImageAlt === endImageAlt && startItem !== e.currentTarget && !usedItems.has(startItem) && !usedItems.has(e.currentTarget)) {
        ...
        successSound.play();  // Reproduce sonido de éxito
        correctCount+=10;
        correctCountElement.textContent = correctCount;
        usedItems.add(startItem);  // Marcar el elemento como usado
        usedItems.add(e.currentTarget);  // Marcar el elemento como usado
    } else {
        console.log("No se puede unir imágenes de diferentes tipos o la misma imagen.");
    }
...
document.getElementById('btn-redirigir').addEventListener('click', function() {
    guardarProgreso(usuarioId, 3, correctCount, timerElement.textContent);
    redirigir();
});
